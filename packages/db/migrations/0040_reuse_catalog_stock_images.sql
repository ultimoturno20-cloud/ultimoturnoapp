...
create index if not exists idx_pricecharting_cache_tcg_sibling_normalized_number
  on pricecharting_cache_entries (
    language_group,
    normalized_expansion,
    (
      case
        when lower(regexp_replace(split_part(coalesce(card_number, ''), '/', 1), '[^a-zA-Z0-9]+', '', 'g')) ~ '^[0-9]+$'
          then coalesce(nullif(ltrim(lower(regexp_replace(split_part(coalesce(card_number, ''), '/', 1), '[^a-zA-Z0-9]+', '', 'g')), '0'), ''), '0')
        else lower(regexp_replace(split_part(coalesce(card_number, ''), '/', 1), '[^a-zA-Z0-9]+', '', 'g'))
      end
    ),
    lower(regexp_replace(product_name, '\s*\[[^]]+\]\s*$', ''))
  );
...
left join lateral (
  ...
  order by case when candidate_pce.product_name !~ '\[' then 0 else 1 end,
    case when coalesce(cie.image_url, '') <> '' then 0 else 1 end,
    cie.match_confidence desc, cie.updated_at desc
  limit 1
) tcg_match on true
left join lateral (
